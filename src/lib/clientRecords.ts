...
export function buildClientRecords(loans: LoanRequest[], users: AdminUser[] = []): ClientRecord[] {
  ...
  for (const [key, clientLoans] of groups) {
    ...
    records.push({
      ...
      disbursementType: latest.disbursement_type ?? profile?.disbursement_type ?? null,
      bankName: latest.bank_name ?? profile?.bank_name ?? null,
      bankAccountName: latest.bank_account_name ?? profile?.bank_account_name ?? null,
      bankAccountNumber: latest.bank_account_number ?? profile?.bank_account_number ?? null,
      bankBranchCode: latest.bank_branch_code ?? profile?.bank_branch_code ?? null,
      bankBranchName: latest.bank_branch_name ?? profile?.bank_branch_name ?? null,
      loans: sorted,
